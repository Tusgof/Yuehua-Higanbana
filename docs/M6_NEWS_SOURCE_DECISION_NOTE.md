...
- **2026-07-03 update**: `docs\GDELT_BULK_RAW_SOURCE_DECISION_NOTE.md` completes News-Unblock N.2 source evaluation. GDELT bulk raw files are approved only for a no-download manifest/size probe next; they do not unblock live LLM research yet.
...
4. If evaluating GDELT bulk raw files, start with a no-download manifest/size probe that maps candidate decision windows to file URLs and expected schema before any raw file download.
5. Do not run DeepSeek/OpenRouter prompt-family research until at least one real timestamp-clean case set is imported and audited.
