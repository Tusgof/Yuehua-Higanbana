...
| `HIGANBANA_DATA_ROOT` | Raw, normalized, derived, and paid market data | Encrypted external disk or versioned object storage | `python scripts/verify_data_integrity.py` |
...
3. Run `python scripts/verify_data_integrity.py` before and after the copy. If legacy paid downloads are not yet represented in `datasets.jsonl`, generate the reconstructable per-file registry once with `python scripts/verify_data_integrity.py --inventory-only --write-supplemental-registry`.
