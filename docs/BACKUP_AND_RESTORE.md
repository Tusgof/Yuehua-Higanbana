...
Higanbana is complete only when all three stores below can be restored together.

| Store | Current role | Backup requirement | Verification |
|:--|:--|:--|:--|
| Higanbana repository | Code, governance, preregistrations, reports, and published Thai research notes under `research_log/` | GitHub remote plus a second clone | `git fsck --full`; hermetic tier; research-log audit |
...
1. Push the Higanbana repository and record its commit hash.
...
1. Clone Higanbana; the tracked `research_log/` directory is restored with it.
