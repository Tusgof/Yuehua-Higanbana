...
- **Untouched inventory result**: `reports/diagnostics/h_a2_orb_0936_untouched_inventory.json` checked local availability, provenance, timestamp policy, and committed dual-hash metadata without parsing target outcomes. All 525 local dates are contaminated by prior development or outcome review, leaving 0 untouched local dates in every VIX, trend, and macro/control bucket. Exact MinTRL remains unknown until a valid timestamp-correct return distribution exists.
- **Current cost-control artifact**: `reports/data_cost/h_a2_orb_0936_cost_plan.json` proposes 20 chronological normal-VIX control dates after `2026-04-24`, with base projection `$12.361983` and a 15% ceiling of `$14.216280`. Trend labels remain pending prior-only SPY daily history. The plan awaits user approval and authorizes no metadata call or purchase.
- **Live cost result**: The user approved the exact cohort, and `reports/data_cost/h_a2_orb_0936_live_cost_estimate.json` priced all 41 requests at `$15.524591`. This exceeds the approved `$14.216280` ceiling by `$1.308311`, although projected cumulative `DATABENTO_API_01` usage `$27.886574` remains below its `$50` cap. The gate stopped before download and no target outcome was parsed.
- **Current H-A2 status**: Active but cost-blocked on fresh data, not on rule definition. The timestamp-correct `09:36` rule is preregistered and the exact 20-date acquisition was approved, but the live estimate exceeded its ceiling. A new user decision must either raise the ceiling or separately revise the cohort. No download or validation run occurred. Neither the invalidated threshold-`0.001` branch nor the entry-before-signal PnL may be reused as edge evidence.
