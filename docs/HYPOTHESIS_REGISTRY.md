...
- **Current 09:36 control artifact**: `experiments/h_a2_orb_0936_preregistration.json` and `docs/H_A2_ORB_0936_PREREGISTRATION.md` lock the replacement question before any new outcome review. Opening range uses bars stamped `09:30` through `09:34`; the confirmation bar stamped `09:35` covers `09:35-09:36`; signal and decision occur at `09:36`; fixed latency is 60 seconds; and the first eligible complete two-leg quote is at `09:37`. Strike selection uses nearest discrete strikes without interpolation, all costs are locked, splits are chronological, OOS tuning is forbidden, and DSR records at least 10 inherited-plus-new trials. This is E0 design evidence only.
- **Untouched inventory result**: Before acquisition, the metadata-only inventory found all 525 local dates contaminated by prior development or outcome review. After acquisition, `reports/diagnostics/h_a2_orb_0936_untouched_inventory.json` identifies the exact 20 downloaded dates as untouched local candidates without parsing target outcomes: all 20 are prior-close VIX 15-25 control days, and trend remains unclassified until the separately preregistered import step. Exact MinTRL remains unknown until a valid timestamp-correct return distribution exists.
- **Historical cost gate**: `reports/data_cost/h_a2_orb_0936_live_cost_estimate.json` correctly blocked the unchanged 41-request cohort at `$15.524591` because it exceeded the original `$14.216280` ceiling. This report remains unchanged as the audit record of the first decision boundary.
- **Revised approval and acquisition**: The user approved a revised ceiling of exactly `$15.524591` without changing the 20 dates or research rules. `reports/data_cost/h_a2_orb_0936_live_cost_estimate_v2.json` passed, and `reports/data_cost/databento_download_result_h_a2_orb_0936.json` records all 41 requests complete, `2,798,604,257` bytes, and projected cumulative `DATABENTO_API_01` usage of `$27.886574 / $50`.
- **Integrity and outcome boundary**: `reports/diagnostics/h_a2_orb_0936_post_download_integrity.json` passed with no blockers and all 41 new artifacts passing dual-hash verification. The downloader records `target_outcomes_parsed=false` and `experiment_run=false`; therefore this remains E0 acquisition evidence only.
- **Current H-A2 status**: Active with fresh timestamp-correct raw data acquired, but no validation result yet. The next step is a separate preregistration for local import, coverage checks, and the locked `09:36` OOS validation before any outcome is read. Neither the invalidated threshold-`0.001` branch nor the entry-before-signal PnL may be reused as edge evidence, and no E2, paper-trading, or operational claim is allowed.
