...
- **Current H-A2 status**: Active but blocked on fresh data, not on rule definition. The timestamp-correct `09:36` decision rule is preregistered, but no untouched local dates remain. The next action is user review of the separate cost plan; no purchase or validation run is authorized in this session. Neither the invalidated threshold-`0.001` branch nor the entry-before-signal PnL may be reused as edge evidence.
