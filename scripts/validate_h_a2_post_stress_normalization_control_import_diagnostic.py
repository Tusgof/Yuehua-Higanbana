...
def validate_h_a2_post_stress_normalization_control_import_diagnostic(
    summary_path: Path = DEFAULT_SUMMARY_PATH,
) -> dict[str, Any]:
    summary = _load_json(summary_path)
    blockers: list[str] = []

    if summary.get("schema_version") != "h_a2_post_stress_normalization_control_import_diagnostic_v1":
    ...
    if summary.get("conclusion") != "เธขเธฑเธเธชเธฃเธธเธเนเธกเนเนเธ”เน":
    ...
    _validate_data_quality_notes(summary, blockers)
...
def _validate_data_quality_notes(summary: dict[str, Any], blockers: list[str]) -> None:
    notes = "\n".join(summary.get("data_quality_notes", []))
...
def _validate_imports(summary: dict[str, Any], blockers: list[str]) -> None:
    ...
    if raw.get("total_actual_bytes") != 1057077119:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate H-A2 post-stress normalization/control import diagnostic output.")
    parser.add_argument("--summary-path", type=Path, default=DEFAULT_SUMMARY_PATH)
    args = parser.parse_args(argv)
    result = validate_h_a2_post_stress_normalization_control_import_diagnostic(args.summary_path)
