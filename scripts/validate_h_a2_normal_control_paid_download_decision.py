...
def validate_h_a2_normal_control_paid_download_decision(
    ...
    if decision.get("schema_version") != "h_a2_normal_control_paid_download_decision_v1":
    ...
    if decision.get("decision") != "approve_low_normal_vix_control_pack_download_after_paid_cost_audit_pass":
        blockers.append("decision_must_approve_low_normal_control_after_audit")
    ...
    if selected_batch.get("batch_id") != "low_normal_vix_control_pack":
        blockers.append("selected_batch_must_be_low_normal_vix_control_pack")
    if selected_batch.get("dates") != expected_dates:
        blockers.append("selected_dates_must_match_low_normal_control_pack")
    ...
    if estimate.get("batch_id") != "low_normal_vix_control_pack":
    ...
    if _round6(estimate.get("cost_result", {}).get("total_estimated_cost_usd")) != 5.398913:
        blockers.append("source_estimated_cost_mismatch")
    if _round6(estimate.get("projected_selected_key_usage_if_downloaded_usd")) != 5.398913:
    ...
    if scope.get("dates") != expected_dates:
        blockers.append("approved_dates_must_match_low_normal_control_pack")
    ...
    if guard.get("selected_key_env") != "DATABENTO_API_MO":
        blockers.append("cost_guard_selected_key_must_be_databento_api_mo")
    if _round6(guard.get("estimated_download_cost_usd")) != 5.398913:
        blockers.append("estimated_download_cost_mismatch")
    if _round6(guard.get("projected_selected_key_usage_if_downloaded_usd")) != 5.398913:
    ...
    for phrase in [
        "Do not download any date outside 2025-02-03 through 2025-02-14",
        "Do not download post_stress_normalization_control_pack",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate the H-A2 normal/control paid download decision.")
