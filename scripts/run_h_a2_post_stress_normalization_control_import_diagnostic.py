...
DEFAULT_PREREG_PATH = PROJECT_ROOT / "experiments" / "h_a2_post_stress_normalization_control_import_diagnostic_preregistration.json"
DEFAULT_DOWNLOAD_RESULT_PATH = (
    PROJECT_ROOT / "reports" / "data_cost" / "databento_download_result_h_a2_post_stress_normalization_control_pack.json"
)
...
DEFAULT_SUMMARY_PATH = PROJECT_ROOT / "reports" / "diagnostics" / "h_a2_post_stress_normalization_control_import_diagnostic.json"
DEFAULT_REPORT_PATH = PROJECT_ROOT / "reports" / "diagnostics" / "h_a2_post_stress_normalization_control_import_diagnostic.md"
DEFAULT_SEARCH_LOG_PATH = (
    PROJECT_ROOT / "reports" / "diagnostics" / "search_logs" / "h_a2_post_stress_normalization_control_import_diagnostic_search_log.jsonl"
)
DEFAULT_BUILD_ROOT = PROJECT_ROOT / "build" / "h_a2_post_stress_normalization_control_pack"


def run_h_a2_post_stress_normalization_control_import_diagnostic(
    ...
    summary = {
        ...
        "schema_version": "h_a2_post_stress_normalization_control_import_diagnostic_v1",
        "experiment_id": "h_a2_post_stress_normalization_control_import_diagnostic",
        "hypothesis_id": "H-A2",
        "status": "complete",
        "evidence_tier": "E1",
        "conclusion": "เธขเธฑเธเธชเธฃเธธเธเนเธกเนเนเธ”เน",
        "conclusion_reason": (
            "The already-downloaded 10-day post-stress normalization/control pack can be parsed locally and used to reconstruct "
        ...
        "data_quality_notes": prereg.get("target_sample", {}).get("data_quality_notes", []),
        "network_used": False,
        ...
        "research_log_slug": "higanbana-h-a2-post-stress-normalization-control-import-diagnostic",
        "research_log_path": "research_log/037-higanbana-h-a2-post-stress-normalization-control-import-diagnostic.md",
        "allowed_claims": [
            "The post-stress normalization/control pack can be parsed locally.",
...
def _load_spy_bars(raw_path: Path, date_text: str) -> list[dict[str, Any]]:
    ...
    for item in frame.sort_index().itertuples():
        ...
        rows.append(
            {
                "record_type": "spy_bar",
                "schema_version": "h_a2_post_stress_normalization_control_spy_bar_v1",
                "date": date_text,
...
def _decision(aggregate: dict[str, Any]) -> dict[str, Any]:
    ...
    if ready_count > 0:
        ...
        next_action = (
            "Pre-register a separate bounded exact-replay diagnostic for the post-stress normalization/control candidate dates only before "
            "computing candidate-trade PnL or making any E2 claim."
        )
    elif signal_count > 0:
        decision = "post_stress_normalization_control_candidate_signal_but_quote_data_blocked"
        next_action = "Diagnose missing entry/forced-close quote availability before any exact replay."
    else:
        decision = "post_stress_normalization_control_no_candidate_trade_signal"
...
def _write_search_log(summary: dict[str, Any], path: Path) -> None:
    ...
    for row in summary["date_diagnostics"]:
        for check_id in [
            ...
            records.append(
                {
                    "record_type": "h_a2_post_stress_normalization_control_import_diagnostic_check",
...
def _render_report(summary: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        "## Import Summary",
        "",
        f"- Data quality notes: `{'; '.join(summary.get('data_quality_notes', [])) or 'none'}`",
...
def main() -> int:
    result = run_h_a2_post_stress_normalization_control_import_diagnostic()
