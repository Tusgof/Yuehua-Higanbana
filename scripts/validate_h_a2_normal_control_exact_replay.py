...
def validate_h_a2_normal_control_exact_replay(summary_path: Path = DEFAULT_SUMMARY_PATH) -> dict[str, Any]:
    summary = _load_json(summary_path)
    blockers: list[str] = []

    if summary.get("schema_version") != "h_a2_normal_control_exact_replay_v1":
    ...
    if summary.get("experiment_id") != "h_a2_normal_control_exact_replay":
...
def _validate_candidate(summary: dict[str, Any], blockers: list[str]) -> None:
    ...
    if candidate.get("date") != CANDIDATE_DATE:
        blockers.append("candidate_date_must_be_2025_02_11")
...
def _validate_selected_vertical(summary: dict[str, Any], blockers: list[str]) -> None:
    ...
    if not isinstance(selected.get("legs"), list) or len(selected["legs"]) != 2:
        ...
        return
    sides = sorted(row.get("side") for row in selected["legs"])
    if sides != ["buy", "sell"]:
...
def _validate_outputs(summary: dict[str, Any], blockers: list[str]) -> None:
    ...
    if summary.get("research_log_slug") != "higanbana-h-a2-normal-control-exact-replay":
    ...
    if not (PROJECT_ROOT / "reports" / "diagnostics" / "h_a2_normal_control_exact_replay.md").exists():
        blockers.append("markdown_report_missing")
    forbidden = "\n".join(summary.get("forbidden_claims", []))
    for phrase in ["edge is validated", "E2", "paper trading", "2025-02-11", "0.001"]:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate H-A2 normal/control exact replay summary.")
    parser.add_argument("--summary-path", type=Path, default=DEFAULT_SUMMARY_PATH)
    args = parser.parse_args(argv)
    result = validate_h_a2_normal_control_exact_replay(args.summary_path)
