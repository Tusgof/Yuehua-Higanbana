...
def validate_h_a2_post_stress_normalization_control_import_diagnostic_preregistration(
    ...
    if (
        ...
        != "h_a2_post_stress_normalization_control_import_diagnostic_preregistration_v1"
    ):
    ...
    if prereg.get("experiment_id") != "h_a2_post_stress_normalization_control_import_diagnostic":
        blockers.append("experiment_id_must_match")
    if (
        prereg.get("source_download_result")
        != "reports/data_cost/databento_download_result_h_a2_post_stress_normalization_control_pack.json"
    ):
...
def _validate_source_download(prereg: dict[str, Any], blockers: list[str]) -> None:
    ...
    if result.get("schema_version") != "h_a2_post_stress_normalization_control_download_result_v1":
    ...
    if result.get("scenario") != "h_a2_post_stress_normalization_control_pack":
    ...
    if _round6(result.get("total_estimated_cost_usd")) != 5.558642:
        blockers.append("source_total_estimated_cost_must_match")
    execution = result.get("execution", {})
    if execution.get("total_bytes") != 1057077119:
        blockers.append("source_total_bytes_must_match_post_stress_download")
    if execution.get("downloaded_count") + execution.get("cache_count", 0) != 20:
        blockers.append("source_execution_must_cover_20_files")
    if execution.get("errors") not in ([], None):
        blockers.append("source_execution_errors_must_be_empty")
    scope = result.get("approved_download_scope", {})
    if scope.get("dates") != EXPECTED_DATES:
        blockers.append("source_dates_must_match_expected_post_stress_dates")
...
def _validate_target_sample(prereg: dict[str, Any], blockers: list[str]) -> None:
    target = prereg.get("target_sample", {})
    if target.get("batch_id") != "post_stress_normalization_control_pack":
        blockers.append("target_batch_id_must_be_post_stress_normalization_control_pack")
    if target.get("scenario") != "h_a2_post_stress_normalization_control_pack":
        blockers.append("target_scenario_mismatch")
    if target.get("date_start") != "2025-05-05":
        blockers.append("target_date_start_must_be_2025_05_05")
    if target.get("date_end") != "2025-05-16":
        blockers.append("target_date_end_must_be_2025_05_16")
    ...
    if target.get("expected_total_bytes") != 1057077119:
        blockers.append("expected_total_bytes_must_match_post_stress_download")
    ...
    if target.get("selected_key_env") != "DATABENTO_API_AI":
        blockers.append("selected_key_env_must_be_databento_api_ai")
    if _round6(target.get("estimated_cost_usd")) != 5.558642:
        blockers.append("estimated_cost_usd_must_match_h_a2_51")
    notes = "\n".join(target.get("data_quality_notes", []))
    if "2025-05-06" not in notes or "reduced-quality" not in notes:
        blockers.append("data_quality_notes_must_preserve_2025_05_06_reduced_quality_warning")
...
def _validate_planned_steps(prereg: dict[str, Any], blockers: list[str]) -> None:
    ...
    first_step = (prereg.get("planned_steps") or [{}])[0]
    if "2025-05-06" not in first_step.get("description", ""):
        blockers.append("planned_steps_must_preserve_2025_05_06_quality_note")
...
def _validate_claims_and_guardrails(prereg: dict[str, Any], blockers: list[str]) -> None:
    ...
    for field in [
        "local_raw_dbn_parse_allowed_after_preregistration",
        "normalization_diagnostic_allowed_after_preregistration",
    ]:
        if guardrails.get(field) is not True:
            blockers.append(f"{field}_must_be_true")

    completion = "\n".join(prereg.get("completion_criteria", []))
    for phrase in ["2025-05-05", "2025-05-16", "SPY bar import", "OPRA quote import", "09:35-only", "0.001", "validator passes"]:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate H-A2 post-stress normalization/control import diagnostic preregistration."
    )
