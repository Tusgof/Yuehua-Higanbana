...
from scripts.validate_h_a2_fresh_oos_cost_plan import DEFAULT_PATH as DEFAULT_COST_PLAN, validate as validate_cost_plan


DEFAULT_PATH = PROJECT_ROOT / "experiments" / "h_a2_fresh_oos_paid_download_decision.json"


def validate(path: Path = DEFAULT_PATH, cost_plan_path: Path = DEFAULT_COST_PLAN) -> dict[str, Any]:
    payload = load_json(path)
    plan = load_json(cost_plan_path)
    blockers: list[str] = []
    scope = payload.get("approved_scope") or {}
    guard = payload.get("cost_guard") or {}
    authorization = payload.get("authorization") or {}
    plan_dates = [date for window in plan.get("target_windows", []) for date in window.get("dates", [])]

    if validate_cost_plan(cost_plan_path).get("status") != "pass":
        blockers.append("source_cost_plan_must_pass")
    if payload.get("status") != "approved" or payload.get("decision") != "approve_exact_20_date_fresh_oos_download_after_live_cost_pass":
        blockers.append("decision_must_be_approved")
    ...
    if scope.get("request_count") != 40:
        blockers.append("request_count_must_be_40")
    ...
    if float(guard.get("approved_purchase_ceiling_usd", -1)) != 12.601188:
    ...
    if authorization.get("broad_calendar_expansion_allowed") is not False:
        blockers.append("broad_expansion_must_be_forbidden")
