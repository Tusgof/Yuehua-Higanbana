...
def validate_h_a2_post_stress_normalization_control_exact_replay_preregistration(
    prereg_path: Path = DEFAULT_PREREG_PATH,
) -> dict[str, Any]:
    prereg = _load_json(prereg_path)
    blockers: list[str] = []

    _require_equal(prereg, "schema_version", "h_a2_post_stress_normalization_control_exact_replay_preregistration_v1", blockers)
    _require_equal(prereg, "artifact_type", "preregistration", blockers)
    _require_equal(prereg, "hypothesis_id", "H-A2", blockers)
    _require_equal(prereg, "status", "preregistered", blockers)
    _require_equal(prereg, "evidence_tier", "E0", blockers)
    _require_equal(prereg, "experiment_id", EXPERIMENT_ID, blockers)
    _require_equal(
        prereg,
        "source_import_diagnostic",
        "reports/diagnostics/h_a2_post_stress_normalization_control_import_diagnostic.json",
        blockers,
    )

    _validate_source_import(blockers)
...
def _validate_source_import(blockers: list[str]) -> None:
    ...
    if summary.get("experiment_id") != "h_a2_post_stress_normalization_control_import_diagnostic":
    ...
    if summary.get("diagnostic_decision", {}).get("decision") != (
        "post_stress_normalization_control_candidate_trade_data_ready_for_separate_preregistered_exact_replay"
    ):
    ...
    if aggregate.get("candidate_trade_data_ready_dates") != [CANDIDATE_DATE]:
        blockers.append("source_candidate_ready_dates_must_be_2025_05_05_only")
...
def _validate_candidate_scope(prereg: dict[str, Any], blockers: list[str]) -> None:
    ...
    expected = {
        "batch_id": "post_stress_normalization_control_pack",
        "scenario": "h_a2_post_stress_normalization_control_pack",
        "candidate_dates": [CANDIDATE_DATE],
        "candidate_count": 1,
        "candidate_direction_by_date": {CANDIDATE_DATE: "call"},
        "blocked_candidate_dates": [],
        "no_signal_dates_count": 9,
        "source_total_dates": 10,
        "source_timestamp_alignment_pass_count": 10,
        "source_opra_quote_rows": 36296592,
        "source_0dte_valid_mid_rows": 939097,
    }
...
def _validate_locked_signal(prereg: dict[str, Any], blockers: list[str]) -> None:
    ...
    expected = {
        ...
        "threshold_must_remain_locked": True,
    }
    for key, value in expected.items():
...
def _validate_outputs_claims_guardrails(prereg: dict[str, Any], blockers: list[str]) -> None:
    ...
    for phrase in ["2025-05-05", "mid_pnl", "implementable_pnl", "09:35", "0.001", "E1 diagnostic"]:
...
def _require_equal(payload: dict[str, Any], key: str, expected: Any, blockers: list[str]) -> None:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate H-A2 post-stress normalization/control exact replay preregistration."
    )
