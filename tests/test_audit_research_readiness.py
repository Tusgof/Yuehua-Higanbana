...
class AuditResearchReadinessTests(unittest.TestCase):
    ...
    def test_current_project_readiness_is_blocked_by_data_and_news(self) -> None:
        ...
        self.assertTrue(any("formally restricted to prior-close VIX <25" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("two independent stress-silence windows" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("fresh OOS 2025-2026 cost plan is ready for user review" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("20 untouched dates" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("user-approval ceiling $12.601188" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("No metadata call or purchase occurred" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("refresh live metadata cost and the selected-key guard" in action for action in result["next_safe_actions"]))
        ...
        self.assertTrue(any("Do not approve E2, paper trading, operational validation, or real-money trading" in action for action in result["next_safe_actions"]))
