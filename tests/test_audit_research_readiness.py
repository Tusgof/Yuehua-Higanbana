...
class AuditResearchReadinessTests(unittest.TestCase):
    ...
    def test_current_project_readiness_is_blocked_by_data_and_news(self) -> None:
        ...
        self.assertTrue(any("H-A2 09:36 ORB live metadata cost gate is blocked with no download" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("41/41 requests were priced at $15.524591" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("above the approved $14.21628 ceiling" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("only blocker is the experiment-specific ceiling" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("Do not download, silently reduce the 20-date cohort, or raise the ceiling" in action for action in result["next_safe_actions"]))
