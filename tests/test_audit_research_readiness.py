...
class AuditResearchReadinessTests(unittest.TestCase):
    ...
    def test_current_project_readiness_is_blocked_by_data_and_news(self) -> None:
        ...
        self.assertTrue(any("H-A2.66 October 2022 exact stress replay is complete" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("13/13 dates pass SPY bar coverage" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("all dates fail the ex-ante clean macro/VIX gate" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("leaving 0 candidate trades" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("stop without the contingent September purchase" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("revisit hypothesis/regime alignment" in action for action in result["next_safe_actions"]))
        ...
        self.assertTrue(any("do not approve paper trading, operational validation, real-money trading, or claim e2" in action.lower() for action in result["next_safe_actions"]))
