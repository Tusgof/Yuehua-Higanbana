...
class AuditResearchReadinessTests(unittest.TestCase):
    ...
    def test_current_project_readiness_is_blocked_by_data_and_news(self) -> None:
        ...
        self.assertTrue(any("H-A2.63 targeted data/regime expansion plan is pre-registered" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("vague sample-size shortage into specific target sets" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("train candidate geometry backfill first" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("H-A2.64" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("cache inventory plus metadata/cost estimate only" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("entry strike mapping" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("MinTRL/PSR coverage" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("Do not download data yet" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("do not broad-buy calendar data" in action for action in result["next_safe_actions"]))
        ...
        self.assertTrue(any("E0 control evidence" in action for action in result["next_safe_actions"]))
        ...
        self.assertTrue(any("OOS rule evaluation" in action for action in result["next_safe_actions"]))
        ...
        self.assertTrue(any("do not approve paper trading, operational validation, real-money trading, or claim E2" in action for action in result["next_safe_actions"]))
        ...
        self.assertTrue(any("H-A2.17 IBKR readiness gate rerun is complete and externally blocked" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("no local IBKR API port is listening on 7497/7496/4002/4001" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("No historical data was requested, no order was transmitted, no paid data was used, and no new provider was introduced" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("start local TWS/Gateway with API enabled" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("Do not treat fixture bars as real data" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("claim H-A2 edge from H-A2.16/H-A2.17" in action for action in result["next_safe_actions"]))
