...
class AuditResearchReadinessTests(unittest.TestCase):
    ...
    def test_current_project_readiness_is_blocked_by_data_and_news(self) -> None:
        ...
        self.assertTrue(any("H-A2 09:36 ORB bounded fresh-data acquisition is complete as E0 evidence" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("41/41 requests for the unchanged 20-date cohort are present" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("inventory identifies 20 untouched local dates" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("committed estimate is $15.524591" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("No target outcomes or PnL were parsed and no experiment ran" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("separate preregistration for local import, coverage checks" in action for action in result["next_safe_actions"]))
        self.assertFalse(any("H-A2 09:36 ORB live metadata cost gate is blocked with no download" in action for action in result["next_safe_actions"]))
        self.assertFalse(any("H-A2 09:36 ORB is pre-registered as E0 design evidence" in action for action in result["next_safe_actions"]))
