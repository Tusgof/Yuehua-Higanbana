...
class AuditResearchReadinessTests(unittest.TestCase):
    ...
    def test_current_project_readiness_is_blocked_by_data_and_news(self) -> None:
        ...
        self.assertTrue(any("H-A2.64 cache inventory is complete" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("Train geometry cache is ready for 30/30 target dates" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("normal/control has 2 candidate-ready dates" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("stress geometry remains blocked by 13 missing real 2022 SPY underlying-bar dates" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("local/no-paid train/control geometry parser" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("Do not call Databento metadata while Technical DD Workstream 1 remains open" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("do not download data" in action for action in result["next_safe_actions"]))
