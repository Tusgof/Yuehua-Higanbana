...
class AuditResearchReadinessTests(unittest.TestCase):
    ...
    def test_current_project_readiness_is_blocked_by_data_and_news(self) -> None:
        ...
        self.assertTrue(any("fresh OOS checkpoint is complete as E1 methodology-failure evidence" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("threshold-0.001 branch is lookahead-contaminated" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("6 retrospective candidates from 20 dates" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("Mechanical PnL, PSR, and MinTRL are invalid for inference" in action for action in result["next_safe_actions"]))
        self.assertTrue(any("observable 09:35 rule or explicit 09:36 rule" in action for action in result["next_safe_actions"]))
