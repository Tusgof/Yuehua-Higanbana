...
from scripts.validate_h_a2_normal_control_import_diagnostic_preregistration import (
    DEFAULT_PREREG_PATH,
...
class H_A2NormalControlImportDiagnosticPreregistrationTests(unittest.TestCase):
    def test_current_preregistration_passes(self) -> None:
        result = validate_h_a2_normal_control_import_diagnostic_preregistration()

        self.assertEqual("pass", result["status"])
        self.assertEqual([], result["blockers"])
        self.assertEqual("H-A2", result["hypothesis_id"])
        self.assertEqual("E0", result["evidence_tier"])
        self.assertEqual("h_a2_normal_control_import_diagnostic", result["experiment_id"])
        self.assertEqual("2025-02-03", result["date_start"])
        self.assertEqual("2025-02-14", result["date_end"])
        self.assertEqual(20, result["expected_file_count"])
        self.assertEqual("DATABENTO_API_MO", result["selected_key_env"])
    ...
    def test_rejects_high_vix_first_resume_permission(self) -> None:
        prereg = json.loads(DEFAULT_PREREG_PATH.read_text(encoding="utf-8"))
        prereg["guardrails"]["resume_high_vix_first_acquisition_allowed"] = True
        result = self._validate_temp(prereg)

        self.assertEqual("blocked", result["status"])
        self.assertIn("resume_high_vix_first_acquisition_allowed_must_be_false", result["blockers"])
    ...
    def _validate_temp(payload: dict) -> dict:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            return validate_h_a2_normal_control_import_diagnostic_preregistration(path)
