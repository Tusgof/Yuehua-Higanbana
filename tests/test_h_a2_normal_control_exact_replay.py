...
from scripts.validate_h_a2_normal_control_exact_replay import (
...
class H_A2NormalControlExactReplayTests(unittest.TestCase):
    ...
    def test_current_summary_validates(self) -> None:
        ...
        result = validate_h_a2_normal_control_exact_replay()
        ...
        self.assertEqual("2025-02-11", result["candidate_date"])
    ...
    def test_validator_rejects_broader_or_changed_candidate(self) -> None:
        payload = self._load_summary()
        payload["candidate"]["date"] = "2025-02-10"
        payload["candidate"]["direction"] = "put"
        result = self._validate_temp(payload)

        self.assertEqual("blocked", result["status"])
        self.assertIn("candidate_date_must_be_2025_02_11", result["blockers"])
    ...
    def _validate_temp(payload: dict) -> dict:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            return validate_h_a2_normal_control_exact_replay(path)
