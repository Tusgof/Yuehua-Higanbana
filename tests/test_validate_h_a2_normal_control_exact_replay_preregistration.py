...
from scripts.validate_h_a2_normal_control_exact_replay_preregistration import (
    DEFAULT_PREREG_PATH,
...
class H_A2NormalControlExactReplayPreregistrationTests(unittest.TestCase):
    def test_current_preregistration_passes(self) -> None:
        result = validate_h_a2_normal_control_exact_replay_preregistration()

        self.assertEqual("pass", result["status"])
        self.assertEqual([], result["blockers"])
        self.assertEqual("H-A2", result["hypothesis_id"])
        self.assertEqual("E0", result["evidence_tier"])
        self.assertEqual("h_a2_normal_control_exact_replay", result["experiment_id"])
        self.assertEqual(["2025-02-11"], result["candidate_dates"])
    ...
    def test_rejects_broader_candidate_dates(self) -> None:
        ...
        prereg["candidate_scope"]["candidate_dates"].append("2025-02-10")
        prereg["candidate_scope"]["candidate_count"] = 2
        result = self._validate_temp(prereg)

        self.assertEqual("blocked", result["status"])
        self.assertIn("candidate_dates_must_be_2025_02_11_only", result["blockers"])
        self.assertIn("candidate_count_must_be_1", result["blockers"])
    ...
    def _validate_temp(payload: dict) -> dict:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            return validate_h_a2_normal_control_exact_replay_preregistration(path)
